...
static BOOL ResolveExecutableMappedSymbol32(
    LPEXECUTABLE_METADATA Metadata, EXECUTABLE_VIRTUAL_ADDRESS_MAPPER Mapper, LPVOID MapperContext, LPCSTR Name,
    LINEAR* Address) {
    ...
    if (SymbolTableAddress == 0 || StringTableAddress == 0 || SymbolTable->SymbolEntrySize == 0) {
        WARNING(
            TEXT("Invalid symbol table mapping name=%s sym=%p str=%p ent=%u"), Name, (LPVOID)SymbolTableAddress,
            (LPVOID)StringTableAddress, SymbolTable->SymbolEntrySize);
        return FALSE;
    }
    ...
    for (UINT SymbolIndex = 0; SymbolIndex < SymbolCount; SymbolIndex++) {
        LPEXECUTABLE_ELF32_SYMBOL Symbol =
            (LPEXECUTABLE_ELF32_SYMBOL)(SymbolTableAddress + SymbolIndex * SymbolTable->SymbolEntrySize);
        LPCSTR SymbolName;
    ...
    WARNING(
        TEXT("Symbol not found name=%s count=%u sym=%p str=%p sym_size=%u str_size=%u bad_name_offset=%u"), Name,
        SymbolCount, (LPVOID)SymbolTableAddress, (LPVOID)StringTableAddress, SymbolTable->SymbolTableSize,
        SymbolTable->StringTableSize, InvalidNameOffsetFound ? 1 : 0);
    ...
    for (UINT SymbolIndex = 0; SymbolIndex < DumpLimit; SymbolIndex++) {
        ...
        DEBUG(
            TEXT("sample idx=%u name_off=%u info=%x shndx=%u value=%x"), SymbolIndex, Symbol->Name, Symbol->Info,
...
static BOOL ResolveExecutableMappedSymbol64(
    ...
    for (UINT SymbolIndex = 0; SymbolIndex < SymbolCount; SymbolIndex++) {
        LPEXECUTABLE_ELF64_SYMBOL Symbol =
            (LPEXECUTABLE_ELF64_SYMBOL)(SymbolTableAddress + SymbolIndex * SymbolTable->SymbolEntrySize);
        LPCSTR SymbolName;
    ...
    for (UINT SymbolIndex = 0; SymbolIndex < DumpLimit; SymbolIndex++) {
        ...
        DEBUG(
            TEXT("sample idx=%u name_off=%u info=%x shndx=%u value=%x"), SymbolIndex, Symbol->Name, Symbol->Info,
            Symbol->SectionIndex, (UINT)Symbol->Value);
    }
...
static LINEAR MapExecutableModuleVirtualAddress(
    ...
    UINT SegmentSizes[EXECUTABLE_MAX_SEGMENTS], UINT VirtualAddress, BOOL RequireWritable) {
...
static BOOL ResolveExecutableModuleSymbol(
    ...
    UINT SymbolIndex, LPCSTR SymbolName, U8 Info, U16 SectionIndex, UINT Value, LINEAR* Address) {
...
static BOOL RelocateExecutableModuleTable32(
    LPEXECUTABLE_MODULE_IMAGE Image, LINEAR SegmentBases[EXECUTABLE_MAX_SEGMENTS],
    UINT SegmentSizes[EXECUTABLE_MAX_SEGMENTS], LPEXECUTABLE_RELOCATION_TABLE_INFO Table,
    EXECUTABLE_SYMBOL_RESOLVER Resolver, LPVOID ResolverContext) {
    ...
    SymbolTableAddress =
        MapExecutableModuleVirtualAddress(Image, SegmentBases, SegmentSizes, SymbolTable->SymbolTableAddress, FALSE);
    StringTableAddress =
        MapExecutableModuleVirtualAddress(Image, SegmentBases, SegmentSizes, SymbolTable->StringTableAddress, FALSE);
    if (TableAddress == 0 || SymbolTableAddress == 0 || StringTableAddress == 0 || Table->EntrySize == 0) {
        WARNING(
            TEXT("[RelocateExecutableModuleTable64] Invalid table mapping table=%p sym=%p str=%p entry_size=%u"),
            (LPVOID)TableAddress, (LPVOID)SymbolTableAddress, (LPVOID)StringTableAddress, Table->EntrySize);
        return FALSE;
    }
    ...
    for (UINT RelocationIndex = 0; RelocationIndex < RelocationCount; RelocationIndex++) {
        ...
        if (Type == R_386_TLS_TPOFF) {
            if (!ApplyExecutableModuleTlsRelocation32(Type, Target, Addend, Image->Metadata.Tls.TotalSize))
                return FALSE;
            continue;
        }
        ...
        if (Type == R_386_RELATIVE) {
            ...
            if (Addend != 0 && SymbolAddress == 0) return FALSE;
                       ...
                       Image, SegmentBases, SegmentSizes, Resolver, ResolverContext, SymbolIndex, SymbolName,
                       Symbol->Info, Symbol->SectionIndex, Symbol->Value, &SymbolAddress)) {
...
static BOOL ApplyExecutableModuleTlsRelocation64(U32 Type, LINEAR Target, UINT Addend, UINT TlsSize) {
    if (Target == 0 || TlsSize == 0) {
        WARNING(
            TEXT("[ApplyExecutableModuleTlsRelocation64] Invalid TLS relocation type=%u target=%p addend=%u "
                 "tls_size=%u"),
            Type, (LPVOID)Target, Addend, TlsSize);
        return FALSE;
    }
...
static BOOL RelocateExecutableModuleTable64(
    ...
    for (UINT RelocationIndex = 0; RelocationIndex < RelocationCount; RelocationIndex++) {
        ...
        if (Table->Type == EXECUTABLE_RELOCATION_TABLE_REL || Table->Type == EXECUTABLE_RELOCATION_TABLE_PLT_REL) {
            ...
            if (Target == 0) {
                WARNING(
                    TEXT("[RelocateExecutableModuleTable64] REL target map failed index=%u offset=%x"), RelocationIndex,
                    (UINT)Offset);
                return FALSE;
            }
            Addend = *((UINT*)Target);
            ...
            if (Target == 0) {
                WARNING(
                    TEXT("[RelocateExecutableModuleTable64] RELA target map failed index=%u offset=%x"),
                    RelocationIndex, (UINT)Offset);
                return FALSE;
            }
        }
        ...
        if (Type == R_X86_64_TPOFF64) {
            if (!ApplyExecutableModuleTlsRelocation64(Type, Target, Addend, Image->Metadata.Tls.TotalSize)) {
                WARNING(
                    TEXT("[RelocateExecutableModuleTable64] TLS relocation failed index=%u offset=%x addend=%u "
                         "tls_size=%u"),
                    RelocationIndex, (UINT)Offset, Addend, Image->Metadata.Tls.TotalSize);
                return FALSE;
            }
            continue;
        }
        ...
        if (Type == R_X86_64_RELATIVE) {
            ...
            if (Addend != 0 && SymbolAddress == 0) {
                WARNING(
                    TEXT("[RelocateExecutableModuleTable64] Relative relocation map failed index=%u addend=%u"),
                    RelocationIndex, Addend);
                return FALSE;
            }
        } else if (!ResolveExecutableModuleSymbol(
                       Image, SegmentBases, SegmentSizes, Resolver, ResolverContext, SymbolIndex, SymbolName,
                       Symbol->Info, Symbol->SectionIndex, (UINT)Symbol->Value, &SymbolAddress)) {
            WARNING(TEXT("Unresolved symbol name=%s"), SymbolName);
            return FALSE;
        }
        ...
        if (!ApplyExecutableModuleRelocation64(Type, Target, SymbolAddress, Addend)) {
            WARNING(
                TEXT("[RelocateExecutableModuleTable64] Apply relocation failed index=%u type=%u"), RelocationIndex,
                Type);
            return FALSE;
        }
    }
