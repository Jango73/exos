SYSTEM_STAGING_DEPENDENCIES = $(KERNEL_BIN) $(PORTAL_ELF) $(NETGET_ELF) $(HELLO_ELF) $(TICTACTOE_ELF) $(TERMTACTICS_ELF) $(INPUTINFO_ELF) $(MEMORY_SMOKE_ELF) $(MASTER_ELF) $(SLAVE_ELF) $(MODULE_HOST_ELF) $(MODULE_SAMPLE_ELF) $(MODULE_IMPORT_ELF) $(PIPE_SOURCE_ELF) $(PIPE_UPPER_ELF) $(PIPE_SUMMARY_ELF) $(SYSTEM_TEST_EPK) $(SYSTEM_SCRIPT_FILES) $(TCC_SAMPLE_FILES) $(TCC_INCLUDE_FILES) $(RUNTIME_INCLUDE_FILES) $(RUNTIME_INCLUDE_SYS_FILES) $(TCC_CRT1_OBJ) $(TCC_CRTI_OBJ) $(TCC_CRTN_OBJ) $(TCC_LIBC_A) $(TCC_LIBC_MIT_LICENSE)
TINYGL_AVAILABLE = $(if $(wildcard ../third/tinygl/include/tinygl.h),1,0)

ifeq ($(TINYGL_AVAILABLE),1)
SYSTEM_STAGING_DEPENDENCIES += $(TINYGL_DEMO_ELF)
endif
...
define STAGE_SYSTEM_EXT2_TREE
	...
	@if [ -f "$(TINYGL_DEMO_ELF)" ]; then cp $(TINYGL_DEMO_ELF) $1/exos/apps/tinygl-demo; fi
	...
	@cp $(PIPE_SOURCE_ELF) $1/exos/apps/test/pipe-source
	@cp $(PIPE_UPPER_ELF) $1/exos/apps/test/pipe-upper
	@cp $(PIPE_SUMMARY_ELF) $1/exos/apps/test/pipe-summary
