...
#include "exos/exos-runtime-http.h"
...
/***************************************************************************/
static int HTTP_ChunkParserProcess(
    HTTP_CHUNK_PARSER* Parser, const unsigned char* Data, unsigned int Length, FILE* File, unsigned int* BytesWritten) {
    ...
    while (Offset < Length && Parser->State != HTTP_CHUNK_STATE_FINISHED) {
        switch (Parser->State) {
            case HTTP_CHUNK_STATE_READ_SIZE: {
                ...
                if (Character == '\n') {
                    if (Parser->SizeBufferUsed > 0 && Parser->SizeBuffer[Parser->SizeBufferUsed - 1] == '\r') {
...
static int HTTP_HeaderValueContainsToken(const char* Value, const char* Token) {
    ...
    while (*Current) {
        ...
        while (Segment[Matched] && Segment[Matched] != ',' && Segment[Matched] != ';' && Matched < TokenLength &&
...
int HTTP_DownloadToFile(
    ...
    while (!responseComplete) {
        ...
        if (!headersParsed) {
            ...
            if (bodyDataInBuffer > 0U) {
                ...
                if (isChunked) {
                    ...
                    result =
                        HTTP_ChunkParserProcess(&chunkParser, bodyStart, bodyDataInBuffer, file, &chunkBytesWritten);
                    if (result != HTTP_SUCCESS) {
        ...
        if (isChunked) {
            unsigned int chunkBytesWritten = 0U;
            result = HTTP_ChunkParserProcess(
                &chunkParser, (const unsigned char*)receiveBuffer, (unsigned int)received, file, &chunkBytesWritten);
            if (result != HTTP_SUCCESS) {
